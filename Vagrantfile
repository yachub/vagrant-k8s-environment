...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "controller01" do |controller01|
    ...
    controller01.vm.provision :shell, :inline => "yum upgrade -y --disablerepo=kubernetes,docker"
  ...
  config.vm.define "controller02" do |controller02|
    ...
    controller02.vm.provision :shell, :inline => "yum upgrade -y --disablerepo=kubernetes,docker"
    ...
    controller02.vm.provision "shell", inline: "sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no controller01.localdomain 'puppet apply /vagrant/puppet/manifests/site.pp'"
    # Something is out of order, a pre-flight check fails, run puppet again to resolve
    controller02.vm.provision :shell, :inline => "puppet apply /vagrant/puppet/manifests/site.pp"
  ...
  config.vm.define "worker01" do |worker01|
    ...
    worker01.vm.provision :shell, :inline => "yum upgrade -y --disablerepo=kubernetes,docker"
  ...
  config.vm.define "worker02" do |worker02|
    ...
    worker02.vm.provision :shell, :inline => "yum upgrade -y --disablerepo=kubernetes,docker"
...
3. Copy the token located in files/service-account-token, select \"Token\" option on the Dashboard, paste the token and click SIGN IN"
  end
end
